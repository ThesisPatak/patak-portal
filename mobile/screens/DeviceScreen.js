...
export default function DeviceScreen({ token, onBack }) {
  ...
  const loadDevices = async () => {
    setLoading(true);
    try {
      const response = await fetch('https://patak-portal-production-351f.up.railway.app/devices/list', {
  ...
  const handleLinkDevice = async (deviceId, deviceToken) => {
    try {
      ...
      const response = await fetch('https://patak-portal-production-351f.up.railway.app/devices/link', {
        method: 'POST',
        headers: {
